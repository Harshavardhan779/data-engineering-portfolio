# Data Engineering Projects Portfolio

Hi! I'm Harsha, a 20-year-old aspiring Data Engineer currently working as a Software Engineer Intern at Azentio.

## About Me
- 🎓 B.Tech in AI/ML from Saveetha Engineering College
- 💼 Currently working with Oracle PL/SQL, Forms, and Reports
- 🚀 Learning Data Engineering, AWS, and building end-to-end pipelines
- 💡 Passionate about turning data into insights

## Technical Skills
- **Languages:** Python, SQL, Java, PL/SQL
- **Data:** Pandas, NumPy, SQL (Oracle, MySQL, PostgreSQL)
- **Cloud:** AWS (S3, RDS, Lambda - learning)
- **Tools:** Git, Jupyter, VS Code
- **Databases:** Oracle, MySQL

## Projects

### 1. Sales Data Analysis Pipeline
*November 2025*

End-to-end ETL pipeline for sales data analysis.
- Extracts data from CSV
- Transforms and calculates metrics
- Generates business insights
- Saves results for reporting

**Tech:** Python, Pandas

[View Project](./sales-pipeline/)

---

## Learning Journey
Following a structured 8-week Data Engineering curriculum:
- ✅ Week 1: AWS Basics, SQL, Python for Data
- 🔄 Week 2: ETL Pipelines, Data Transformation
- ⏳ Week 3-4: Cloud Data Engineering
- ⏳ Week 5-6: Advanced Topics
- ⏳ Week 7-8: Real-world Projects

## DSA Practice
Solving coding problems daily to strengthen problem-solving skills.
- LeetCode Profile: https://leetcode.com/u/harshavardhan779/
- HackerRank Profile:[ [link]](https://www.hackerrank.com/profile/harshavardhanbv1)
- Problems Solved: 3+ and counting!

## Connect With Me
- 📧 Email: harshavardhanbv779@gmail.com
- 💼 LinkedIn: www.linkedin.com/in/harshavardhan779
- 📊 GitHub: You're already here!

---

*"Building my data engineering skills one project at a time!"*